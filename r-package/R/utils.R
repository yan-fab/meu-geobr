...
download_parquet <- function(filename_to_download,
  ...
  # --- Local DB: check before any network request ---
  # Parse geography, year, and type from filename_to_download.
  # Expected pattern: {geography}_{year}_{simplified|original}.parquet
  m <- regmatches(
    filename_to_download,
    regexec("^(.+)_(\\d{4})_(simplified|original)\\.parquet$", filename_to_download)
  )[[1]]

  if (length(m) == 4) {
    local_geography <- m[2]
    local_year      <- as.integer(m[3])
    local_simplified <- m[4] == "simplified"

    local_path <- resolve_local_path(
      geography  = local_geography,
      year       = local_year,
      simplified = local_simplified
    )

    if (!is.null(local_path)) {
      cli::cli_alert_info("Using local file: {.path {local_path}}")
      return(geobr_open_dataset(local_path))
    }
  }

  # --- Normal pipeline: download from network ---
