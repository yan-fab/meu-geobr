...
def read_geobr_v2(
    ...
    If a local database has been configured via ``geobr.set_local_db()``, the
    function will first look for the requested dataset locally and skip the
    network download entirely when found.
    """
    from geobr._local_db import _resolve_local
    ...
    # --- Local DB: check before attempting any network request ---
    local_path = _resolve_local(geography, year, simplified)
    if local_path is not None:
        ...
        return convert_output(relation, output=output, connection=conn)
